...
Shape_S::Shape_S(Falling_shapes* const fs_ptr, Field* const f_ptr) :
	Shape(S_COLOR, fs_ptr, f_ptr, { COORD{f_ptr->get_width() / 2, BEGIN_Y - 2}, COORD{ f_ptr->get_width() / 2 + 1, BEGIN_Y - 2},
...
bool Shape_S::rotate()
{
	...
	{
	case Position::POS1:
		tmp.edit_coord({ COORD{1,1}, COORD{0,2}, COORD{1,-1}, COORD{0,0} });
		pos = Position::POS2;
		break;
	case Position::POS2:
