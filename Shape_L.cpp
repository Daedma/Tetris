...
Shape_L::Shape_L(Falling_shapes* const fs_ptr, Field* const f_ptr) :
	Shape(L_COLOR, fs_ptr, f_ptr, { COORD{f_ptr->get_width() / 2, BEGIN_Y - 3}, COORD{ f_ptr->get_width() / 2, BEGIN_Y - 2},
...
bool Shape_L::rotate()
{
	...
	case Position::POS1:
		...
		break;
	case Position::POS2:
		tmp.edit_coord({ COORD{1,-1}, COORD{0,0}, COORD{-1,1}, COORD{0,-2} });
		pos = Position::POS3;
		break;
	case Position::POS3:
		...
		break;
	case Position::POS4:
		tmp.edit_coord({ COORD{0,2}, COORD{1,-1}, COORD{0,0}, COORD{-1,1} });
		pos = Position::POS1;
		break;
	default:
		break;
