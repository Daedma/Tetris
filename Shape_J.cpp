...
Shape_J::Shape_J(Falling_shapes* const fs_ptr, Field* const f_ptr) :
	Shape(J_COLOR, fs_ptr, f_ptr, { COORD{f_ptr->get_width() / 2 + 1, BEGIN_Y - 3}, COORD{ f_ptr->get_width() / 2 + 1, BEGIN_Y - 2},
...
bool Shape_J::rotate()
{
	...
	{
	case Position::POS1:
		tmp.edit_coord({ COORD{1,1}, COORD{0,0}, COORD{0,-2}, COORD{-1,-1} });
		pos = Position::POS2;
		break;
	case Position::POS2:
		...
		break;
	case Position::POS3:
		tmp.edit_coord({ COORD{1,1}, COORD{0,2}, COORD{0,0}, COORD{-1,-1} });
		pos = Position::POS4;
		break;
	case Position::POS4:
		tmp.edit_coord({ COORD{1,-1}, COORD{0,0}, COORD{-1,1}, COORD{-2,0} });
		pos = Position::POS1;
		break;
	default:
		break;
